#!/usr/bin/env bash
# Validasi Exercise Sesi 1: index exercise-server-monitoring ada dengan
# minimal 5 dokumen, cluster sehat.
set -uo pipefail
...
if [ "$count" -ge 5 ] 2>/dev/null; then
  check "Index exercise-server-monitoring punya minimal 5 dokumen (count=$count)" 0
else
  check "Index exercise-server-monitoring punya minimal 5 dokumen (count=$count)" 1
fi
