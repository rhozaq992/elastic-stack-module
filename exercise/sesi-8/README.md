...
**Kerjakan exercise ini SELAGI cluster Sesi 8 masih berjalan** (sesi ini
adalah sesi terakhir pelatihan — matikan cluster setelah selesai untuk
...
```
```bash
curl -X PUT "http://localhost:9200/_snapshot/lab-fs-repo/snapshot-exercise-8?wait_for_completion=true" \
  -H 'Content-Type: application/json' \
  -d '{ "indices": "exercise-cluster-backup", "ignore_unavailable": true }'

curl -X DELETE "http://localhost:9200/exercise-cluster-backup"

curl -X POST "http://localhost:9200/_snapshot/lab-fs-repo/snapshot-exercise-8/_restore?wait_for_completion=true" \
  -H 'Content-Type: application/json' \
...
curl -X POST "http://localhost:9200/_snapshot/lab-fs-repo/snapshot-exercise-8/_restore" \
  -H 'Content-Type: application/json' \
  -d '{ "indices": "exercise-cluster-backup", "include_global_state": false }'
...
Validasi hasil pekerjaan Anda (jalankan SELAGI cluster Sesi 8 masih aktif):
```bash
